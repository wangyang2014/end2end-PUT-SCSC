...
class Datasets(object):
    def __init__(self, mix_reader, target_reader_list, mvn_dict='cmvn.dict', apply_log=True,pre_train=False):
    ...
    def _transform(self, mixture_specs, targets_specs_list):
        ...
        source_attr["spectrogram"] = th.tensor(
            mixture_specs)
        target_attr["spectrogram"] = [
            th.tensor(t) for t in targets_specs_list
        ]
        # 
        status = [1,1]
        if self.pre_train:
            ...
            if index == 0:
                ...
                input_spectra = np.abs(targets_specs_list[0]) if np.iscomplexobj(
                targets_specs_list[0]) else targets_specs_list[0]
                status = [1,0]
            else:
                input_spectra = np.abs(targets_specs_list[1]) if np.iscomplexobj(
                targets_specs_list[1]) else targets_specs_list[1]
                status = [0,1]
        return {
